...
class SimpleModel(keras.Model):
    def __init__(self, vocab_dict: Dict[str, int], emb_names: List[str],
                 dim_list: List[int], num_classes: int) -> None:
        super(SimpleModel, self).__init__(name='SimpleModel')
        self._feature_layer = self._build_feature_layer(
            vocab_dict, emb_names, dim_list)
    ...
    def _build_feature_layer(self, vocab_dict: Dict[str, int],
                             emb_names: List[str], dim_list: List[int]
        ...
        Arguments:
            ...
            emb_names {List[str]} -- embedding column names
            dim_list {List[int]} -- dimension for each embedding column

        Returns:
            keras.layers.DenseFeatures -- first layer for network
        """
        emb_columns: List = []
        for name, dim in zip(emb_names, dim_list):
