...
class TransProbFM(nn.Module):
    def __init__(self,
                 feature_dim: int,
                 num_dim: int,
                 init_mean: float,
                 init_scale: float = 0.001):
        super(TransProbFM, self).__init__()

        var_linear = T.rand(feature_dim, 1,
                            dtype=T.double) * init_mean * 2 - init_mean

        loc_emb = T.rand(feature_dim, num_dim,
                         dtype=T.double) * init_mean * 2 - init_mean
        param_loc_emb = nn.Parameter(loc_emb)
        scale_emb = T.ones_like(loc_emb) * init_scale
        param_scale_emb = nn.Parameter(scale_emb)
        norm_emb = dist.Normal(loc=param_loc_emb, scale=param_scale_emb)

        loc_trans = T.rand(feature_dim, num_dim,
                           dtype=T.double) * init_mean * 2 - init_mean
        param_loc_trans = nn.Parameter(loc_trans)
        scale_trans = T.ones_like(loc_trans) * init_scale
        param_scale_trans = nn.Parameter(scale_trans)
        norm_trans = dist.Normal(loc=param_loc_trans, scale=param_scale_trans)

        self._feature_dim = feature_dim
        self._num_dim = num_dim
        self._init_mean = init_mean
        self.param_linear = nn.Parameter(var_linear)
        self.param_emb = param_loc_emb
        self.param_scale_emb = param_scale_emb
        self.param_trans = param_loc_trans
        self.param_scale_trans = param_scale_trans
        self.norm_emb = norm_emb
        self.norm_trans = norm_trans

    def forward(self, pos_feats: T.Tensor, neg_feats: T.tensor) -> T.Tensor:
        feature_dim = self._feature_dim
        param_linear = self.param_linear
        param_emb = self.norm_emb.rsample()
        param_trans = self.norm_trans.rsample()
...
if __name__ == "__main__":
    model = TransProbFM(120, 100, 1.)
    model.to(T.cuda.current_device())
    model(None, None)
